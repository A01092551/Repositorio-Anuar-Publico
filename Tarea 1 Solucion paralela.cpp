...
#include <cstdlib> // Librería para generar números aleatoreos
#include <ctime>   // Librería que proporciona funciones relacionadas con la fecha y el tiempo.

#define N 100000000 // Tamaño de los arreglos
#define chunk 1   // Pedazos para resolver la suma
...
{
    std::srand(std::time(0)); // semilla para generar números aleatoreos
    ...
    for (i = 0;i < N; i++) //Se usa un for para asignar un valores a todos los elementos de los arreglos a y b
                           // los valores de los elementos se asignan de manera aleatoria entre el 0 y el 100
    {
        a[i] = std::rand() % 101;
        b[i] = std::rand() % 101;
    }
